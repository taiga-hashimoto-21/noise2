...
# ─── 評価用 Test Loss を算出（ΔEαとΔEβ の MSEの平均）─────
model.eval()
with torch.no_grad():
    preds = model(X)
    targets = Y
    mse_alpha = ((preds[:, 0] - targets[:, 0])**2).mean().item()
    mse_beta = ((preds[:, 1] - targets[:, 1])**2).mean().item()
    test_loss = (mse_alpha + mse_beta) / 2

print(f"\n📉 Test Loss は ΔEαとΔEβの2乗誤差の平均（MSE）です")
print(f"　→ Test Loss = (ΔEα_MSE + ΔEβ_MSE) / 2 = ({mse_alpha:.6f} + {mse_beta:.6f}) / 2 = {test_loss:.6f}")